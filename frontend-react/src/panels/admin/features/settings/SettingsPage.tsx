...
import { api, apiGet, getApiErrorMessage, getValidationErrors } from '@common/services/api';
...
export function SettingsPage() {
  ...
  useEffect(() => {
    apiGet<Record<string, string>>(adminApi.settings)
      .then((settings) => {
        setForm({
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await api.put(adminApi.settings, { settings: form });
