...
import { apiPage, toRows, type LaravelPage } from '@common/services/api';
import type { ResourceName, ResourceRecord } from '@common/types';
...
export function useResourceRecords(resource: ResourceName) {
  ...
  const endpoint = useMemo(() => config.endpoint, [config.endpoint]);
